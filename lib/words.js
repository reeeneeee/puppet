...
export function isRealWord(word, corrections = null) {
  ...
  if (WORDS.has(cleaned)) return true;
  ...
  // Check common inflections: strip suffix and see if base is known
  const suffixes = [
    // plurals & verb forms
    "'s", "s", "es", "ies",
    // past tense / participles
    "ed", "ied", "ing",
    // doubling: e.g. "running" → "run", "stopped" → "stop"
  ...
  for (const suffix of suffixes) {
    if (cleaned.endsWith(suffix) && cleaned.length > suffix.length + 1) {
      ...
      if (stem.length >= 2 && stem[stem.length - 1] === stem[stem.length - 2]) {
        if (WORDS.has(stem.slice(0, -1))) return true;
      }
      // Handle -ies → -y: "babies" → "bab" + y → "baby"
      if (suffix === "ies" && WORDS.has(stem + "y")) return true;
      // Handle -ied → -y: "carried" → "carr" + y → "carry"... check with undoubled too
      if (suffix === "ied") {
        if (WORDS.has(stem + "y")) return true;
      }
      // Handle silent-e drop: "making" → "mak" + e → "make"
      if ((suffix === "ing" || suffix === "ed" || suffix === "er" || suffix === "est") && WORDS.has(stem + "e")) return true;
      // Handle -ily → -y: "happily" → "happ" + y → "happy"
      if (suffix === "ily" && WORDS.has(stem + "y")) return true;
    }
  }
